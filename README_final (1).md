...
An initial broader query matched the substring `"ren"` to catch file rename activity, producing false positives from unrelated processes (Edge WebView, WhatsApp Desktop). The query was tuned to remove this overly broad condition — a real example of reducing false positives, a core SOC analyst skill.

### Detection Gap Identified
Shell built-in commands (`echo`, `ren`) did not generate separate Event ID 4688 entries — a known limitation. **Recommendation:** use Sysmon Event ID 11/23 or File Integrity Monitoring for full file-level visibility in production.
