...
### What I Did
- Created fake scheduled task named `WindowsUpdate` — classic attacker disguise technique
- Task configured to run `cmd.exe /c whoami` on every logon as SYSTEM
- Used Splunk Event ID 4104 as alternative detection
- Detected task name and creation activity in PowerShell logs
- Deleted fake task after detection

### Detection Note
> Event ID 4698 requires Task Scheduler Operational log source. As an alternative, PowerShell Script Block Logging (Event ID 4104) was used to detect the task creation command. In a production SOC environment, both should be monitored together.
