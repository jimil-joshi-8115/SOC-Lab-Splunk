...
- Identified who cleared the logs, when and from which machine
- Demonstrated why SIEM is critical — Splunk preserved all evidence even after logs were cleared!
...
### Result
Splunk detected **2 log clearing events** by account `hp` on host `JIMIL-JOSHI` with message **"The audit log was cleared"** — confirming anti-forensics activity.
